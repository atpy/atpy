...
def list_tables(cursor,dbtype):
    ...
    if dbtype=='sqlite':
        ...
        cursor.execute("SELECT table_name FROM information_schema.tables WHERE table_schema NOT IN ('pg_catalog', 'information_schema');")
...
def column_info(cursor,dbtype,table_name):
    ...
    names,types = [],[]
    if dbtype=='sqlite':
        for column in cursor.execute('pragma table_info('+table_name+')').fetchall():
...
def create_table(cursor,dbtype,table_name,column_names,column_types):
    ...
    for i,column_name in enumerate(column_names):
        ...
        query += quote[dbtype]+column_name+quote[dbtype]+" "+column_type
